...
function UserPage({ isAuth: isAuthProp }) {
  ...
  const {
    ...
    otherFollowersPagination: useSelector(
      selectors.selectOtherFollowersPagination
    ),
  ...
  useEffect(() => {
    ...
    setTabs(newTabs);
    setActiveTab(prev =>
      newTabs.includes(prev) ? prev : newTabs[0] || 'RECIPES'
    );
  ...
  const resolvedRecipesPagination = isCurrentUser
    ? recipesPagination
    : otherRecipesPagination;
